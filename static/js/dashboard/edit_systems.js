...
let currentGeocodingCities = [];
let currentGeocodingRoads = [];
    ...
    systemSelect.addEventListener("change", (e) => {
        ...
        fetch(`/api/systems?radio_system_id=${selectedId}&include_config=1`)
            ...
            .then(({result}) => {
                ...
                populateUpdateNtfy(system_data);
...
function initFormListeners() {
    ...
    // NTFY TAB
    // ---------------------------------------------------------------------
    on("updateSystemNtfyForm", "submit", async (ev) => {
        ev.preventDefault();
        await saveNtfySettings();
    });
    ...
    // Cities modals
    on("addressCityAddBtn", "click", (ev) => {
        ev.preventDefault();
        openAddressCityModalForAdd();
    });

    on("addressCityForm", "submit", async (ev) => {
        ev.preventDefault();
        await saveAddressCityFromModal(ev.target);
    });

    on("addressCityDeleteForm", "submit", async (ev) => {
        ev.preventDefault();
        await deleteAddressCity();
    });

    // Bounds buttons
    on("addressBoundsComputeBtn", "click", async (ev) => {
        ev.preventDefault();
        await computeAddressBoundsFromCalls();
    });

    on("addressBoundsFitMapBtn", "click", (ev) => {
        ev.preventDefault();
        fitBoundsMapToInputs();
    });

    // When Address tab is shown, init map and invalidate size
    // (Leaflet can't measure a hidden div, so we defer until visible)
    document.getElementById("address-tab")?.addEventListener("shown.bs.tab", () => {
        initBoundsMapPicker();
        // Give browser a tick to render, then resize + fit
        requestAnimationFrame(() => {
            if (_boundsMap) {
                _boundsMap.invalidateSize();
                fitBoundsMapToInputs();
            }
        });
    });
...
function populateUpdateAddressExtraction(system_data) {
    ...
    // Bounds
    const bMinLat = document.getElementById("addressBoundsMinLat");
    const bMaxLat = document.getElementById("addressBoundsMaxLat");
    const bMinLng = document.getElementById("addressBoundsMinLng");
    const bMaxLng = document.getElementById("addressBoundsMaxLng");
    if (bMinLat) bMinLat.value = s.bounds_min_lat ?? "";
    if (bMaxLat) bMaxLat.value = s.bounds_max_lat ?? "";
    if (bMinLng) bMinLng.value = s.bounds_min_lng ?? "";
    if (bMaxLng) bMaxLng.value = s.bounds_max_lng ?? "";
    // Map init deferred until tab is shown (Leaflet can't measure hidden div)
    ...
    currentGeocodingCities = Array.isArray(s.cities) ? [...s.cities] : [];
    populateGeocodingCitiesTable(currentGeocodingCities, {
        onEdit:   openAddressCityModalForEdit,
...
async function refreshAddressExtraction() {
    ...
    currentGeocodingRoads = Array.isArray(s.roads) ? [...s.roads] : [];
    populateGeocodingRoadsTable(currentGeocodingRoads, {
        onEdit:   openAddressRoadModalForEdit,
        onDelete: confirmDeleteAddressRoad,
        onMove:   reorderAddressRoad
    });
}
...
function gatherAddressExtractionSettingsFormData() {
    ...
    const boundsMinLat = q("#addressBoundsMinLat").trim();
    const boundsMaxLat = q("#addressBoundsMaxLat").trim();
    const boundsMinLng = q("#addressBoundsMinLng").trim();
    const boundsMaxLng = q("#addressBoundsMaxLng").trim();
    if (boundsMinLat !== "") data.bounds_min_lat = parseFloat(boundsMinLat);
    if (boundsMaxLat !== "") data.bounds_max_lat = parseFloat(boundsMaxLat);
    if (boundsMinLng !== "") data.bounds_min_lng = parseFloat(boundsMinLng);
    if (boundsMaxLng !== "") data.bounds_max_lng = parseFloat(boundsMaxLng);
    ...
    if (Array.isArray(currentGeocodingCities) && currentGeocodingCities.length > 0) {
        data.cities = currentGeocodingCities.map(c => ({
...
function openAddressRegionModalForAdd() {
    ...
    const bulkWrap = document.getElementById("addressRegionBulkWrapper");
    if (bulkWrap) bulkWrap.classList.remove("d-none");
...
function openAddressRegionModalForEdit(region_id) {
    ...
    const bulkWrap = document.getElementById("addressRegionBulkWrapper");
    if (bulkWrap) bulkWrap.classList.add("d-none");
...
function gatherAddressRegionFormData(form) {
    ...
    return {
        ...
        bulk            : q("#addressRegionBulk").trim() || null
    };
}
...
async function saveAddressRegionFromModal(formEl) {
    ...
    // --- Bulk add path ---
    if (!data.region_id && data.bulk) {
        const regions = [];
        const parts = data.bulk.split(/[,;]/);
        for (const part of parts) {
            const trimmed = part.trim();
            if (!trimmed) continue;
            const colonIdx = trimmed.indexOf(":");
            if (colonIdx < 0) {
                showAlert(`Invalid bulk entry: "${trimmed}". Use STATE:County format.`, "warning");
                return;
            }
            const state = trimmed.substring(0, colonIdx).trim().toUpperCase();
            const county = trimmed.substring(colonIdx + 1).trim();
            if (!state || !county) {
                showAlert(`Invalid bulk entry: "${trimmed}". Both state and county are required.`, "warning");
                return;
            }
            regions.push({ state_code: state, county_name: county });
        }
        if (!regions.length) {
            showAlert("No valid regions found in bulk input.", "warning");
            return;
        }

        const resp = await apiJson(
            `/api/systems/${sysId}/address_extraction/regions/bulk`,
            {
                method: "POST",
                body: { _csrf_token: data._csrf_token, regions }
            }
        );
        if (!resp.success) {
            showAlert(resp.message || "Failed to bulk-add regions.", "danger");
            return;
        }
        hideModalById("#addressRegionModal");
        showAlert(resp.message || `Added ${resp.result} region(s).`, "success");
...
/*****************************************************
 *  Address Extraction – Cities table + modals
 *****************************************************/
function populateGeocodingCitiesTable(cities, opts = {}) {
    const {
        showActions = true,
        onEdit      = () => {},
        onDelete    = () => {},
    ...
    const tbody = document.querySelector("#addressCitiesTable tbody");
    if (!tbody) return;
    tbody.innerHTML = "";
        ...
        tbody.innerHTML = `
            <tr>
                <td colspan="4" class="text-center text-body-secondary py-3">
                    No cities defined. Click “Add City” to create one.
                </td>
            </tr>`;
        return;
    }
    ...
    sorted.forEach((c, idx) => {
        ...
        if (showActions) {
            actionsHtml = `
                <div class="btn-group btn-group-sm" role="group">
                    <button type="button"
                            class="btn btn-outline-secondary js-ac-move-up"
                            data-id="${c.city_id}" ${upDisabled}
                            title="Move up">
                        <i class="bi bi-arrow-up"></i>
                    </button>
                    <button type="button"
                            class="btn btn-outline-secondary js-ac-move-down"
                            data-id="${c.city_id}" ${downDisabled}
                            title="Move down">
                        <i class="bi bi-arrow-down"></i>
                    </button>
                    <button type="button"
                            class="btn btn-outline-primary js-ac-edit"
                            data-id="${c.city_id}"
                            title="Edit">
                        <i class="bi bi-pencil"></i>
                    </button>
                    <button type="button"
                            class="btn btn-outline-danger js-ac-del"
                            data-id="${c.city_id}"
                            title="Delete">
                        <i class="bi bi-trash"></i>
                    </button>
    ...
    tbody.querySelectorAll(".js-ac-edit")
        .forEach(btn => btn.addEventListener("click",
            () => onEdit(btn.dataset.id)));
    tbody.querySelectorAll(".js-ac-del")
        .forEach(btn => btn.addEventListener("click",
            () => onDelete(btn.dataset.id)));
    tbody.querySelectorAll(".js-ac-move-up")
        .forEach(btn => btn.addEventListener("click",
            () => onMove(btn.dataset.id, -1)));
    tbody.querySelectorAll(".js-ac-move-down")
        .forEach(btn => btn.addEventListener("click",
            () => onMove(btn.dataset.id, +1)));
}

function openAddressCityModalForAdd() {
    const form = document.getElementById("addressCityForm");
    if (!form) return;

    form.reset();

    form.querySelector("#addressCityId").value       = "";
    form.querySelector("#addressCitySystemId").value = currentSystemId ?? "";

    const maxPriority = currentGeocodingCities.reduce(
        (mx, c) => Math.max(mx, c.priority ?? 0), 0
    );
    form.querySelector("#addressCityPriority").value = maxPriority + 10;

    const modalEl = document.getElementById("addressCityModal");
    if (modalEl) modalEl.querySelector(".modal-title").textContent = "Add City";

    const bulkWrap = document.getElementById("addressCityBulkWrapper");
    if (bulkWrap) bulkWrap.classList.remove("d-none");
...
function openAddressCityModalForEdit(city_id) {
    const city = currentGeocodingCities.find(
        c => String(c.city_id) === String(city_id)
    );
    if (!city) return;
    ...
    form.querySelector("#addressCityId").value       = city.city_id;
    form.querySelector("#addressCitySystemId").value = currentSystemId ?? "";
    form.querySelector("#addressCityName").value     = city.city_name ?? "";
    form.querySelector("#addressCityPriority").value = city.priority ?? 0;

    const modalEl = document.getElementById("addressCityModal");
    if (modalEl) modalEl.querySelector(".modal-title").textContent = "Edit City";
...
async function saveAddressCityFromModal(form) {
    if (!currentSystemId) return;

    const cityId = form.querySelector("#addressCityId").value;
    const cityName = (form.querySelector("#addressCityName").value || "").trim();
    const priority = parseInt(form.querySelector("#addressCityPriority").value || "0", 10);
    const bulkText = (form.querySelector("#addressCityBulk")?.value || "").trim();
    const csrf = form.querySelector('[name="_csrf_token"]')?.value || "";
    ...
    if (!cityId && bulkText) {
        ...
        for (const part of parts) {
            ...
            if (!name) continue;
            cities.push({ city_name: name });
        }
        if (!cities.length) {
            showAlert("No valid cities found in bulk input.", "warning");
            return;
        }

        const resp = await apiJson(
            `/api/systems/${currentSystemId}/address_extraction/cities/bulk`,
            {
                method: "POST",
                body: { _csrf_token: csrf, cities }
            }
        );
        if (!resp.success) {
            showAlert(resp.message || "Failed to bulk-add cities.", "danger");
            return;
        }
        hideModalById("#addressCityModal");
        showAlert(resp.message || `Added ${resp.result} city(ies).`, "success");
        await refreshAddressExtraction();
        return;
    }
    ...
    // --- Single add/edit path ---
    if (!cityName) {
        showAlert("City name is required.", "warning");
        return;
    }
    ...
    let url, method;
    if (cityId) {
        url = `/api/systems/${currentSystemId}/address_extraction/cities/${cityId}`;
        method = "PATCH";
    } else {
        url = `/api/systems/${currentSystemId}/address_extraction/cities`;
        method = "POST";
    }

    const resp = await apiJson(url, { method, body: payload });

    if (!resp.success) {
        showAlert(resp.message || "Failed to save city.", "danger");
        return;
    }

    hideModalById("#addressCityModal");
    showAlert(resp.message || "City saved.", "success");
    await refreshAddressExtraction();
}

function confirmDeleteAddressCity(city_id) {
    const city = currentGeocodingCities.find(
        c => String(c.city_id) === String(city_id)
    );
    if (!city) return;

    const deleteId = document.getElementById("addressCityDeleteId");
    const question = document.getElementById("addressCityDeleteQuestion");

    if (deleteId) deleteId.value = city.city_id;
    if (question) question.textContent = `Delete city "${city.city_name}" permanently?`;

    const m = bootstrap.Modal.getOrCreateInstance(document.getElementById("addressCityDeleteModal"));
    m.show();
}

async function deleteAddressCity() {
    const cityId = byId("addressCityDeleteId").value;
    const csrf = document.querySelector("#addressCityDeleteForm [name=_csrf_token]")?.value || "";

    const resp = await apiJson(
        `/api/systems/${currentSystemId}/address_extraction/cities/${cityId}`,
        {
            method: "DELETE",
            body  : { _csrf_token: csrf }
        }
    ...
    if (!resp.success) {
        showAlert(resp.message || "Failed to delete city.", "danger");
        return;
    }

    hideModalById("#addressCityDeleteModal");
    showAlert(resp.message || "City deleted.", "success");

    await refreshAddressExtraction();
}

function reorderAddressCity(city_id, delta) {
    const sorted = [...currentGeocodingCities].sort(
        (a, b) => (a.priority ?? 0) - (b.priority ?? 0)
    );
    const idx = sorted.findIndex(
        c => String(c.city_id) === String(city_id)
    ...
    sorted.forEach((c, i) => c.priority = (i + 1) * 10);
    currentGeocodingCities = sorted;
    ...
    persistAddressCityOrder(sorted.map(c => c.city_id));
}

async function persistAddressCityOrder(orderedIds) {
    if (!currentSystemId) return;

    const csrf = document.querySelector("#updateSystemAddressExtractionForm [name=_csrf_token]")?.value || "";

    const resp = await apiJson(
        `/api/systems/${currentSystemId}/address_extraction/cities/reorder`,
        {
            method: "POST",
            body  : { _csrf_token: csrf, order: orderedIds }
        }
    ...
    if (!resp.success) {
        showAlert(resp.message || "Failed to reorder cities.", "danger");
        return;
    }

    currentGeocodingCities = Array.isArray(resp.result)
        ? resp.result
        : currentGeocodingCities;
    ...
    showAlert(resp.message || "City order saved.", "success");
}

/*****************************************************
 *  Address Extraction – Bounds map picker
 *****************************************************/
let _boundsMap = null;
let _boundsRect = null;

function initBoundsMapPicker() {
    const mapDiv = document.getElementById("boundsMapPicker");
    if (!mapDiv || _boundsMap) return;

    // Default center: Ottawa-ish
    _boundsMap = L.map(mapDiv).setView([45.25, -76.5], 8);
    ...
    // When map is clicked, update bounds inputs to a small box around click
    _boundsMap.on('click', (e) => {
        const lat = e.latlng.lat;
        const lng = e.latlng.lng;
        const pad = 0.05;
        setBoundsInputs(lat - pad, lat + pad, lng - pad, lng + pad);
        drawBoundsRect();
    });
}

function setBoundsInputs(minLat, maxLat, minLng, maxLng) {
    const bMinLat = document.getElementById("addressBoundsMinLat");
    const bMaxLat = document.getElementById("addressBoundsMaxLat");
    const bMinLng = document.getElementById("addressBoundsMinLng");
    const bMaxLng = document.getElementById("addressBoundsMaxLng");
    if (bMinLat) bMinLat.value = minLat.toFixed(6);
    if (bMaxLat) bMaxLat.value = maxLat.toFixed(6);
    if (bMinLng) bMinLng.value = minLng.toFixed(6);
    if (bMaxLng) bMaxLng.value = maxLng.toFixed(6);
}

function fitBoundsMapToInputs() {
    if (!_boundsMap) return;

    const bMinLat = parseFloat(document.getElementById("addressBoundsMinLat")?.value);
    const bMaxLat = parseFloat(document.getElementById("addressBoundsMaxLat")?.value);
    const bMinLng = parseFloat(document.getElementById("addressBoundsMinLng")?.value);
    const bMaxLng = parseFloat(document.getElementById("addressBoundsMaxLng")?.value);

    if ([bMinLat, bMaxLat, bMinLng, bMaxLng].some(v => Number.isNaN(v))) {
        // No valid bounds; fit to Ontario/Quebec area as fallback
        _boundsMap.fitBounds([[44.0, -80.0], [47.0, -74.0]]);
        if (_boundsRect) {
            _boundsMap.removeLayer(_boundsRect);
            _boundsRect = null;
        }
        return;
    }

    const bounds = L.latLngBounds([[bMinLat, bMinLng], [bMaxLat, bMaxLng]]);
    _boundsMap.fitBounds(bounds, { padding: [20, 20] });
    drawBoundsRect();
}
...
function drawBoundsRect() {
    ...
    _boundsRect = L.rectangle(
        [[bMinLat, bMinLng], [bMaxLat, bMaxLng]],
        { color: '#0d6efd', weight: 2, fillOpacity: 0.1 }
    ).addTo(_boundsMap);
}

async function computeAddressBoundsFromCalls() {
    if (!currentSystemId) return;

    const csrf = document.querySelector("#updateSystemAddressExtractionForm [name=_csrf_token]")?.value || "";

    const resp = await apiJson(
        `/api/systems/${currentSystemId}/address_extraction/bounds/compute`,
        {
            method: "POST",
            body  : { _csrf_token: csrf, padding: 0.05 }
        }
    );

    if (!resp.success) {
        showAlert(resp.message || "Failed to compute bounds.", "danger");
        return;
    }

    const b = resp.result || {};
    setBoundsInputs(b.bounds_min_lat, b.bounds_max_lat, b.bounds_min_lng, b.bounds_max_lng);
    fitBoundsMapToInputs();

    showAlert(resp.message || "Bounds computed from calls.", "success");
}
 ...
 *               NTFY TAB
 *****************************************************/
function populateUpdateNtfy(system_data) {
    const n = system_data.ntfy ?? {};

    const ntfyTabTitle = document.getElementById("ntfyTabTitle");
    const updateNtfySystemId = document.getElementById("updateNtfySystemId");
    const updateNtfySystemName = document.getElementById("updateNtfySystemName");

    if (ntfyTabTitle) ntfyTabTitle.textContent = `Ntfy Settings - ${system_data.system_name}`;
    if (updateNtfySystemId) updateNtfySystemId.value = system_data.radio_system_id;
    if (updateNtfySystemName) updateNtfySystemName.value = system_data.system_name;

    const ntfySelect = document.getElementById("ntfySelect");
    const ntfyServerUrl = document.getElementById("ntfyServerUrl");
    const ntfyTopic = document.getElementById("ntfyTopic");
    const ntfyToken = document.getElementById("ntfyToken");
    const ntfyTitleTmpl = document.getElementById("ntfyTitleTmpl");
    const ntfyBodyTmpl = document.getElementById("ntfyBodyTmpl");

    if (ntfySelect) ntfySelect.value = Number(n.enabled) === 1 ? "1" : "0";
    if (ntfyServerUrl) ntfyServerUrl.value = n.server_url ?? "";
    if (ntfyTopic) ntfyTopic.value = n.topic ?? "";
    if (ntfyToken) ntfyToken.value = n.token ?? "";
    if (ntfyTitleTmpl) ntfyTitleTmpl.value = n.title_tmpl ?? "";
...
function gatherNtfySettingsFormData() {
    const form = document.getElementById("updateSystemNtfyForm");
    if (!form) return null;
    const q = sel => form.querySelector(sel)?.value ?? "";

    return {
        _csrf_token     : q('[name=_csrf_token]'),
        radio_system_id : Number(q("#updateNtfySystemId")),
        ntfy_enabled    : Number(q("#ntfySelect")),
        ntfy_server_url : q("#ntfyServerUrl").trim() || null,
        ntfy_topic      : q("#ntfyTopic").trim()      || null,
        ntfy_token      : q("#ntfyToken").trim()      || null,
        ntfy_title_tmpl : q("#ntfyTitleTmpl").trim()  || null,
        ntfy_body_tmpl  : q("#ntfyBodyTmpl").trim()   || null
    };
}

async function saveNtfySettings() {
    const data = gatherNtfySettingsFormData();
    if (!data) return;

    const url  = `/api/systems/${data.radio_system_id}/ntfy/settings`;
    const resp = await apiJson(url, { method: "PATCH", body: data });

    if (!resp.success) {
        showAlert(resp.message || "Failed to save Ntfy settings.", "danger");
        return;
    }

    populateUpdateNtfy({
        radio_system_id : data.radio_system_id,
        system_name     : document.getElementById("updateNtfySystemName")?.value ?? "",
        ntfy            : resp.result
    });

    showAlert(resp.message || "Ntfy settings saved.", "success");
}
...
const INTEGRATION_TESTS = [
    ...
    },
    {
        key: "ntfy",
        btn: "#ntfySendTestBtn",
        form: "#updateSystemNtfyForm",
        endpoint: (id) => `/api/systems/${id}/ntfy/test`,
        canRun: () => {
            const url = val("#ntfyServerUrl");
            const topic = val("#ntfyTopic");
            return !!(url && topic);
        },
        watch: ["#ntfyServerUrl", "#ntfyTopic"]
    }
...
function populateGeocodingRoadsTable(roads, opts = {}) {
    const {
        showActions = true,
        onEdit      = () => {},
        onDelete    = () => {},
    ...
    const tbody = document.querySelector("#addressRoadsTable tbody");
    if (!tbody) return;
    tbody.innerHTML = "";
        ...
        tbody.innerHTML = `
            <tr>
                <td colspan="6" class="text-center text-body-secondary py-3">
                    No roads defined. Click “Add Road” or “Fetch from OSM” to create one.
                </td>
            </tr>`;
        return;
    }
        ...
        if (showActions) {
            actionsHtml = `
                <div class="btn-group btn-group-sm" role="group">
                    <button type="button"
                            class="btn btn-outline-secondary js-ar-move-up"
                            data-id="${r.road_id}" ${upDisabled}
                            title="Move up">
                        <i class="bi bi-arrow-up"></i>
                    </button>
                    <button type="button"
                            class="btn btn-outline-secondary js-ar-move-down"
                            data-id="${r.road_id}" ${downDisabled}
                            title="Move down">
                        <i class="bi bi-arrow-down"></i>
                    </button>
                    <button type="button"
...
function openAddressRoadModalForAdd() {
    const form = document.getElementById("addressRoadForm");
    if (!form) return;

    form.reset();

    form.querySelector("#addressRoadId").value       = "";
    form.querySelector("#addressRoadSystemId").value = currentSystemId ?? "";
    form.querySelector("#addressRoadType").value     = "";
    form.querySelector("#addressRoadCity").value     = "";

    const maxPriority = currentGeocodingRoads.reduce(
        (mx, r) => Math.max(mx, r.priority ?? 0), 0
    );
    form.querySelector("#addressRoadPriority").value = maxPriority + 10;

    const modalEl = document.getElementById("addressRoadModal");
    if (modalEl) modalEl.querySelector(".modal-title").textContent = "Add Road";

    const bulkWrap = document.getElementById("addressRoadBulkWrapper");
    if (bulkWrap) bulkWrap.classList.remove("d-none");
...
function openAddressRoadModalForEdit(road_id) {
    const road = currentGeocodingRoads.find(
        r => String(r.road_id) === String(road_id)
    );
    if (!road) return;
    ...
    form.querySelector("#addressRoadId").value       = road.road_id;
    form.querySelector("#addressRoadSystemId").value = currentSystemId ?? "";
    form.querySelector("#addressRoadName").value     = road.road_name ?? "";
    form.querySelector("#addressRoadType").value     = road.road_type ?? "";
    form.querySelector("#addressRoadCity").value     = road.city_name ?? "";
    form.querySelector("#addressRoadPriority").value = road.priority ?? 0;

    const modalEl = document.getElementById("addressRoadModal");
    if (modalEl) modalEl.querySelector(".modal-title").textContent = "Edit Road";
...
async function saveAddressRoadFromModal(form) {
    if (!currentSystemId) return;

    const roadId   = form.querySelector("#addressRoadId").value;
    const roadName = (form.querySelector("#addressRoadName").value || "").trim();
    const roadType = (form.querySelector("#addressRoadType").value || "").trim() || null;
    const cityName = (form.querySelector("#addressRoadCity").value || "").trim() || null;
    const priority = parseInt(form.querySelector("#addressRoadPriority").value || "0", 10);
    const bulkText = (form.querySelector("#addressRoadBulk")?.value || "").trim();
    const csrf = form.querySelector('[name="_csrf_token"]')?.value || "";

    // --- Bulk add path ---
    if (!roadId && bulkText) {
        const roads = parseAddressRoadBulkInput(bulkText);
        if (!roads.length) {
            showAlert("No valid roads found in bulk input.", "warning");
            return;
        }

        const resp = await apiJson(
            `/api/systems/${currentSystemId}/address_extraction/roads/bulk`,
            {
                method: "POST",
                body: { _csrf_token: csrf, roads }
            }
        );
        if (!resp.success) {
            showAlert(resp.message || "Failed to bulk-add roads.", "danger");
            return;
        }
        hideModalById("#addressRoadModal");
        showAlert(resp.message || `Added ${resp.result} road(s).`, "success");
        await refreshAddressExtraction();
        return;
    }
    ...
    // --- Single add/edit path ---
    if (!roadName) {
        showAlert("Road name is required.", "warning");
        return;
    }
    ...
    let url, method;
    if (roadId) {
        url = `/api/systems/${currentSystemId}/address_extraction/roads/${roadId}`;
        method = "PATCH";
    } else {
        url = `/api/systems/${currentSystemId}/address_extraction/roads`;
        method = "POST";
    }
    ...
    if (!resp.success) {
        showAlert(resp.message || "Failed to save road.", "danger");
        return;
    }

    hideModalById("#addressRoadModal");
    showAlert(resp.message || "Road saved.", "success");
    await refreshAddressExtraction();
}

function parseAddressRoadBulkInput(text) {
    const roads = [];
    ...
    for (const line of lines) {
        ...
        roads.push({
            road_name: name,
            road_type: parts[1]?.trim() || null,
            city_name: parts[2]?.trim() || null,
        });
    }
    return roads;
}

function confirmDeleteAddressRoad(road_id) {
    const road = currentGeocodingRoads.find(
        r => String(r.road_id) === String(road_id)
    );
    if (!road) return;

    const deleteId = document.getElementById("addressRoadDeleteId");
    const question = document.getElementById("addressRoadDeleteQuestion");

    if (deleteId) deleteId.value = road.road_id;
    if (question) question.textContent = `Delete road "${road.road_name}" permanently?`;

    const m = bootstrap.Modal.getOrCreateInstance(document.getElementById("addressRoadDeleteModal"));
    m.show();
}

async function deleteAddressRoad() {
    const roadId = byId("addressRoadDeleteId").value;
    const csrf = document.querySelector("#addressRoadDeleteForm [name=_csrf_token]")?.value || "";

    const resp = await apiJson(
        `/api/systems/${currentSystemId}/address_extraction/roads/${roadId}`,
        {
            method: "DELETE",
            body  : { _csrf_token: csrf }
        }
    ...
    if (!resp.success) {
        showAlert(resp.message || "Failed to delete road.", "danger");
        return;
    }

    hideModalById("#addressRoadDeleteModal");
    showAlert(resp.message || "Road deleted.", "success");

    await refreshAddressExtraction();
}

function reorderAddressRoad(road_id, delta) {
    const sorted = [...currentGeocodingRoads].sort(
        (a, b) => (a.priority ?? 0) - (b.priority ?? 0)
    );
    const idx = sorted.findIndex(
        r => String(r.road_id) === String(road_id)
    ...
    const orderedIds = sorted.map(r => r.road_id);
    persistAddressRoadOrder(orderedIds);
}

async function persistAddressRoadOrder(orderedIds) {
    if (!currentSystemId) return;
    const csrf = document.querySelector("#updateSystemAddressExtractionForm [name=_csrf_token]")?.value || "";

    const resp = await apiJson(
        `/api/systems/${currentSystemId}/address_extraction/roads/reorder`,
        {
            method: "POST",
            body: { _csrf_token: csrf, ordered_ids: orderedIds }
        }
    );

    if (!resp.success) {
        showAlert(resp.message || "Failed to reorder roads.", "danger");
        return;
    }

    currentGeocodingRoads = Array.isArray(resp.result)
        ? resp.result
        : currentGeocodingRoads;
...
// ======================================================================
// OSM Road Fetch Modal
// ======================================================================

let _roadFetchMap = null;
let _roadFetchRect = null;
let _previewRoads = [];

function initRoadFetchMap() {
    const mapDiv = document.getElementById("addressRoadFetchMap");
    if (!mapDiv || _roadFetchMap) return;

    _roadFetchMap = L.map(mapDiv).setView([45.25, -76.5], 8);
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
        attribution: '&copy; OpenStreetMap contributors'
    }).addTo(_roadFetchMap);

    // Draw rectangle when map is clicked (auto-create bounds)
    _roadFetchMap.on('click', (e) => {
        const lat = e.latlng.lat;
        const lng = e.latlng.lng;
        const pad = 0.05;
        setRoadFetchBounds(lat - pad, lat + pad, lng - pad, lng + pad);
        drawRoadFetchRect();
    });
}

function setRoadFetchBounds(minLat, maxLat, minLng, maxLng) {
    const elMinLat = document.getElementById("addressRoadFetchMinLat");
    const elMaxLat = document.getElementById("addressRoadFetchMaxLat");
    const elMinLng = document.getElementById("addressRoadFetchMinLng");
    const elMaxLng = document.getElementById("addressRoadFetchMaxLng");
    if (elMinLat) elMinLat.value = minLat;
    if (elMaxLat) elMaxLat.value = maxLat;
    if (elMinLng) elMinLng.value = minLng;
    if (elMaxLng) elMaxLng.value = maxLng;
}

function getRoadFetchBounds() {
    const minLat = parseFloat(document.getElementById("addressRoadFetchMinLat")?.value);
    const maxLat = parseFloat(document.getElementById("addressRoadFetchMaxLat")?.value);
    const minLng = parseFloat(document.getElementById("addressRoadFetchMinLng")?.value);
    const maxLng = parseFloat(document.getElementById("addressRoadFetchMaxLng")?.value);
    return { minLat, maxLat, minLng, maxLng };
}

function drawRoadFetchRect() {
    const b = getRoadFetchBounds();
    if (isNaN(b.minLat) || isNaN(b.maxLat) || isNaN(b.minLng) || isNaN(b.maxLng)) return;

    if (_roadFetchRect) {
        _roadFetchMap.removeLayer(_roadFetchRect);
    }
    _roadFetchRect = L.rectangle(
        [[b.minLat, b.minLng], [b.maxLat, b.maxLng]],
        { color: '#0d6efd', weight: 2, fillOpacity: 0.1 }
    ).addTo(_roadFetchMap);
    _roadFetchMap.fitBounds(_roadFetchRect.getBounds());
}

function fitRoadFetchMapToInputs() {
    if (!_roadFetchMap) initRoadFetchMap();
    drawRoadFetchRect();
}

async function fetchOsmRoadsPreview() {
    if (!currentSystemId) return;

    const b = getRoadFetchBounds();
    if (isNaN(b.minLat) || isNaN(b.maxLat) || isNaN(b.minLng) || isNaN(b.maxLng)) {
        showAlert("Please set all four bounds values first.", "warning");
        return;
    }
    if (b.minLat >= b.maxLat || b.minLng >= b.maxLng) {
        showAlert("Invalid bounds: min must be less than max.", "warning");
        return;
    }
    ...
    const resp = await apiJson(
        `/api/systems/${currentSystemId}/address_extraction/roads/fetch-osm`,
        {
            method: "POST",
            body: {
                _csrf_token: csrf,
    ...
    if (!resp.success) {
        showAlert(resp.message || "Failed to fetch roads from OSM.", "danger");
        return;
    }

    _previewRoads = Array.isArray(resp.result) ? resp.result : [];

    const wrapper = document.getElementById("addressRoadFetchPreviewWrapper");
    const countEl = document.getElementById("addressRoadFetchCount");
    const tbody = document.querySelector("#addressRoadFetchPreviewTable tbody");
    const importBtn = document.getElementById("addressRoadFetchImportBtn");

    if (wrapper) wrapper.classList.remove("d-none");
    if (countEl) countEl.textContent = `${_previewRoads.length} roads discovered`;
    if (importBtn) importBtn.disabled = _previewRoads.length === 0;

    if (!tbody) return;
    tbody.innerHTML = "";

    if (_previewRoads.length === 0) {
        tbody.innerHTML = `
            <tr>
                <td colspan="4" class="text-center text-body-secondary py-3">
    ...
    _previewRoads.forEach((r, idx) => {
        const row = document.createElement("tr");
        row.innerHTML = `
            <td><input type="checkbox" class="form-check-input js-road-preview-check" data-index="${idx}" checked></td>
    ...
    // Select-all handler
    const selectAll = document.getElementById("addressRoadFetchSelectAll");
    if (selectAll) {
        selectAll.checked = true;
        selectAll.onclick = () => {
            document.querySelectorAll(".js-road-preview-check").forEach(cb => {
...
async function importSelectedOsmRoads() {
    if (!currentSystemId || !_previewRoads.length) return;

    const selected = [];
    document.querySelectorAll(".js-road-preview-check").forEach((cb, idx) => {
        if (cb.checked && _previewRoads[idx]) {
            selected.push({
                road_name: _previewRoads[idx].name,
                road_type: _previewRoads[idx].type,
                city_name: _previewRoads[idx].city,
            });
        }
    });

    if (!selected.length) {
        showAlert("No roads selected for import.", "warning");
        return;
    }
    ...
    const resp = await apiJson(
        ...
        {
            method: "POST",
            body: { _csrf_token: csrf, roads: selected }
        }
    );

    if (!resp.success) {
        showAlert(resp.message || "Failed to import roads.", "danger");
        return;
    }

    const modalEl = document.getElementById("addressRoadFetchModal");
    const m = bootstrap.Modal.getOrCreateInstance(modalEl);
    m.hide();

    showAlert(resp.message || `Imported ${resp.result} road(s).`, "success");
    await refreshAddressExtraction();
}

// Wire up event listeners for road forms
document.addEventListener("DOMContentLoaded", () => {
    // Road add/edit form
    const roadForm = document.getElementById("addressRoadForm");
    if (roadForm) {
        roadForm.addEventListener("submit", async (e) => {
            e.preventDefault();
            await saveAddressRoadFromModal(roadForm);
        });
    }

    // Road delete form
    const roadDelForm = document.getElementById("addressRoadDeleteForm");
    if (roadDelForm) {
        roadDelForm.addEventListener("submit", async (e) => {
            e.preventDefault();
            await deleteAddressRoad();
        });
    }

    // Road fetch modal buttons
    const fetchPreviewBtn = document.getElementById("addressRoadFetchPreviewBtn");
    if (fetchPreviewBtn) {
        fetchPreviewBtn.addEventListener("click", fetchOsmRoadsPreview);
    }

    const fetchFitBtn = document.getElementById("addressRoadFetchFitMapBtn");
    if (fetchFitBtn) {
        fetchFitBtn.addEventListener("click", fitRoadFetchMapToInputs);
    }

    const fetchImportBtn = document.getElementById("addressRoadFetchImportBtn");
    if (fetchImportBtn) {
        fetchImportBtn.addEventListener("click", importSelectedOsmRoads);
    }

    // Init fetch map when modal is shown
    const fetchModal = document.getElementById("addressRoadFetchModal");
    if (fetchModal) {
        fetchModal.addEventListener("shown.bs.modal", () => {
            initRoadFetchMap();
            // Pre-fill bounds from existing settings
            const bMinLat = document.getElementById("addressBoundsMinLat")?.value;
            const bMaxLat = document.getElementById("addressBoundsMaxLat")?.value;
            const bMinLng = document.getElementById("addressBoundsMinLng")?.value;
            const bMaxLng = document.getElementById("addressBoundsMaxLng")?.value;
            if (bMinLat && bMaxLat && bMinLng && bMaxLng) {
                setRoadFetchBounds(bMinLat, bMaxLat, bMinLng, bMaxLng);
                drawRoadFetchRect();
            }
        });
    }
    ...
    // Add Road button
    const addRoadBtn = document.getElementById("addressRoadAddBtn");
    if (addRoadBtn) {
        addRoadBtn.addEventListener("click", openAddressRoadModalForAdd);
    }
});
