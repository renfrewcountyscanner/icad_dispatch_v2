...
from lib.system_module import (
    ...
    bulk_add_geocoding_regions, get_system_storage_settings, update_system_storage_settings,
...
def get_mute_state():
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT COALESCE(SUM(mute_notifications), 0) > 0 FROM radio_systems")
    muted = cursor.fetchone()[0] == 1
    conn.close()
...
def toggle_mute():
    ...
    cursor.execute("SELECT COALESCE(SUM(mute_notifications), 0) FROM radio_systems")
    current_muted = cursor.fetchone()[0] > 0
    new_muted = not current_muted
    
    cursor.execute("UPDATE radio_systems SET mute_notifications = 1" if new_muted else "UPDATE radio_systems SET mute_notifications = 0")
    conn.commit()
    conn.close()
