...
def _in_clause(col: str, values: List[Any]) -> Tuple[str, List[Any]]:
    """
    Build a SQL IN (...) clause for SQLite/PostgreSQL with '?' placeholders.
    Returns a fallback that matches nothing if values is empty.
    ...
    """
    if not values:
        return "1=0", []  # match nothing, safe in all SQL dialects
...
def add_system(db: PostgreSQLDatabase, system_data: dict) -> dict:
    """
...
def add_system_default_settings(db: PostgreSQLDatabase, radio_system_id: int) -> None:
    ...
    # 13) ntfy settings
    ok = db.execute_commit(
        "INSERT INTO radio_system_ntfy_settings (radio_system_id) VALUES (?)",
        (radio_system_id,),
    )
    if not ok["success"]:
        raise RuntimeError(f"ntfy settings: {ok['message']}")

    module_logger.info("Default settings inserted for radio_system_id=%s", radio_system_id)


def delete_system(db: PostgreSQLDatabase, radio_system_id: int | None = None, system_decimal: int | None = None) -> dict:
    """
...
def get_systems(
    ...
    if not include_config:
        ...
        base = """
               SELECT
                   ...
                   rsae.bounds_min_lat                  AS address_extraction_bounds_min_lat,
                   rsae.bounds_max_lat                  AS address_extraction_bounds_max_lat,
                   rsae.bounds_min_lng                  AS address_extraction_bounds_min_lng,
                   rsae.bounds_max_lng                  AS address_extraction_bounds_max_lng,
                   ...
                   -- ntfy
                   rsnt.ntfy_setting_id         AS ntfy_setting_id,
                   rsnt.ntfy_enabled            AS ntfy_enabled,
                   rsnt.ntfy_server_url         AS ntfy_server_url,
                   rsnt.ntfy_topic              AS ntfy_topic,
                   rsnt.ntfy_token              AS ntfy_token,
                   rsnt.ntfy_title_tmpl         AS ntfy_title_tmpl,
                   rsnt.ntfy_body_tmpl          AS ntfy_body_tmpl,
               ...
               FROM radio_systems AS rs
                        ...
                        LEFT JOIN radio_system_n8n_settings      rsn8n ON rs.radio_system_id = rsn8n.radio_system_id
                         ...
                         LEFT JOIN radio_system_ntfy_settings rsnt ON rs.radio_system_id = rsnt.radio_system_id \
               """
    ...
    # PostgreSQL: no GROUP BY needed because settings tables have 1:1 relationship
    tail = " ORDER BY rs.system_name"
    ...
    for row in rows:
        ...
        if include_config:
            ...
            sys_obj["address_extraction"] = {
                ...
                "bounds_min_lat": row.get("address_extraction_bounds_min_lat"),
                "bounds_max_lat": row.get("address_extraction_bounds_max_lat"),
                "bounds_min_lng": row.get("address_extraction_bounds_min_lng"),
                "bounds_max_lng": row.get("address_extraction_bounds_max_lng"),
                "regions": [],
                "cities": [],
            }
            ...
            if address_extraction_setting_id is not None:
                ...
                cities_res = get_geocoding_cities(db, address_extraction_setting_id=address_extraction_setting_id)
                if cities_res.get("success"):
                    sys_obj["address_extraction"]["cities"] = [
                        {
                            "city_id": r["city_id"],
                            "city_name": r["city_name"],
                            "priority": r["priority"],
                        }
                        for r in cities_res["result"]
                    ]
                else:
                    module_logger.error(
                        "Error loading geocoding cities for address_extraction_setting_id=%s: %s",
                        address_extraction_setting_id,
                        cities_res.get("message"),
                    )
            ...
            # Ntfy
            sys_obj["ntfy"] = {
                "ntfy_setting_id": row.get("ntfy_setting_id"),
                "enabled": int(row.get("ntfy_enabled") or 0),
                "server_url": row.get("ntfy_server_url"),
                "topic": row.get("ntfy_topic"),
                "token": row.get("ntfy_token"),
                "title_tmpl": row.get("ntfy_title_tmpl"),
                "body_tmpl": row.get("ntfy_body_tmpl"),
            }
...
# ===========================================================================
def update_system_general(db: PostgreSQLDatabase, system_data: dict) -> dict:
    """
...
# Tone settings
# ===========================================================================
def update_system_tone_settings(db: PostgreSQLDatabase, data: dict) -> dict:
    """
...
# Telegram
# ===========================================================================
def update_system_telegram_settings(db: PostgreSQLDatabase, system_data: dict) -> dict:
    """
...
# Ntfy
# ===========================================================================
def update_system_ntfy_settings(db: PostgreSQLDatabase, system_data: dict) -> dict:
    """
    Update Ntfy settings for a system.
    """
    ...
    q_update = """
        UPDATE radio_system_ntfy_settings
        SET ntfy_enabled      = ?,
            ntfy_server_url   = ?,
            ntfy_topic        = ?,
            ntfy_token        = ?,
        ...
        """
    params_update = (
        _bool_like(system_data.get("ntfy_enabled")),
        system_data.get("ntfy_server_url") or None,
        system_data.get("ntfy_topic") or None,
        system_data.get("ntfy_token") or None,
        system_data.get("ntfy_title_tmpl") or None,
        system_data.get("ntfy_body_tmpl") or None,
        rsid,
    )

    res = db.execute_commit(q_update, params_update, return_count=True)
    if not res.get("success"):
    ...
    if affected == 0:
        q_insert = """
            INSERT INTO radio_system_ntfy_settings (
                radio_system_id,
                ntfy_enabled,
                ntfy_server_url,
                ntfy_topic,
    ...
    return {"success": True, "message": "Ntfy settings updated.", "result": []}


# ===========================================================================
# Discord
# ===========================================================================
def update_system_discord_settings(db: PostgreSQLDatabase, system_data: dict) -> dict:
    """
    ...
    # adapt the extraction below to your PostgreSQLDatabase wrapper if needed.
    affected = 0
    try:
        affected = int(res.get("result") or 0)
    except Exception:
...
def _get_discord_setting_id(db: PostgreSQLDatabase, radio_system_id: int) -> dict:
    """
...
def add_or_update_system_discord_field(db: PostgreSQLDatabase, field_data: dict):
    """
...
def reorder_system_discord_fields(db: PostgreSQLDatabase, discord_setting_id: int, ordered_ids: list[int]):
    """
...
# Make settings
# ===========================================================================
def update_system_make_settings(db: PostgreSQLDatabase, payload: dict):
    """
...
def get_system_make_fields(db: PostgreSQLDatabase, make_setting_id: int, enabled: int | bool | None = None):
    """
...
def add_or_update_system_make_field(db: PostgreSQLDatabase, data: dict):
    """
...
def delete_system_make_field(db: PostgreSQLDatabase, payload_field_id: int):
    """
...
def ensure_system_n8n_row(db: PostgreSQLDatabase, radio_system_id: int) -> dict:
...
def get_system_n8n_settings(db: PostgreSQLDatabase, radio_system_id: int) -> dict:
    """
...
def update_system_n8n_settings(db: PostgreSQLDatabase, data: dict) -> dict:
    """
...
# Pushover
# ===========================================================================
def update_system_pushover_settings(db: PostgreSQLDatabase, system_data: dict):
    """
...
# ===========================================================================
def update_system_email_settings(db: PostgreSQLDatabase, payload: dict) -> dict:
    """
...
def add_or_update_system_email(db: PostgreSQLDatabase, email_data: dict):
    """
...
# ===========================================================================
def update_system_upload_settings(db: PostgreSQLDatabase, payload: dict) -> dict:
    """
...
# ===========================================================================
def update_system_address_extraction_settings(db: PostgreSQLDatabase, data: dict) -> dict:
    """
    ...
    if "address_extraction_enabled" in data:
        ...
        if enabled_bool:
            ...
            if not openai_key:
                return {
                    "success": False,
                    "message": "Cannot enable address extraction without openai_api_key",
                    "result": []
                }
    ...
    # --- Handle bounds (floats; empty string / None → NULL) ---
    for key in ("bounds_min_lat", "bounds_max_lat", "bounds_min_lng", "bounds_max_lng"):
        if key in data:
            v = data.get(key)
            if v is None or v == "":
                norm[key] = None
                ...
                try:
                    ...
                    return {"success": False, "message": f"{key} must be a valid number.", "result": []}
...
def get_system_address_extraction_settings(
    ...
    for row in rows:
        ...
        if include_regions:
            ...
            setting["cities"] = []
            cities_res = get_geocoding_cities(
                db,
                address_extraction_setting_id=row["address_extraction_setting_id"]
            )
            if cities_res.get("success"):
            ...
            setting["roads"] = []
            roads_res = get_geocoding_roads(
                db,
                address_extraction_setting_id=row["address_extraction_setting_id"]
            )
            if roads_res.get("success"):
                setting["roads"] = [
                    {
                        "road_id": r["road_id"],
...
def add_or_update_geocoding_region(db: PostgreSQLDatabase, data: dict) -> dict:
    """
...
def get_geocoding_cities(
    ...
    """
    Retrieve geocoding cities with optional filters.
    """
    base = """
           SELECT
               gc.city_id,
               gc.address_extraction_setting_id,
               gc.city_name,
               gc.priority
    ...
    if city_id is not None:
        conditions.append("gc.city_id = ?")
        params.append(city_id)

    if address_extraction_setting_id is not None:
        conditions.append("gc.address_extraction_setting_id = ?")
        params.append(address_extraction_setting_id)

    if radio_system_id is not None:
        base += " JOIN radio_system_address_extraction_settings aes ON gc.address_extraction_setting_id = aes.address_extraction_setting_id"
        conditions.append("aes.radio_system_id = ?")
        params.append(radio_system_id)

    where = " WHERE " + " AND ".join(conditions) if conditions else ""
    query = base + where + " ORDER BY gc.priority ASC, gc.city_name"

    return db.execute_query(query, tuple(params), fetch_mode="all" if city_id is None else "one")


def add_or_update_geocoding_city(db: PostgreSQLDatabase, data: dict) -> dict:
    """
    Insert or update a geocoding city.

    UPDATE path: pass 'city_id'
    INSERT path: omit 'city_id'
    ...
    if city_id is not None:
        if not city_name:
            return {"success": False, "message": "city_name required.", "result": []}

        q = """
            UPDATE geocoding_cities
            SET city_name = ?, priority = ?
            WHERE city_id = ? \
            """
        return db.execute_commit(q, (city_name, priority, city_id), return_count=True)
    ...
    if not city_name:
        return {"success": False, "message": "city_name is required.", "result": []}

    # Check for duplicate
    dup = db.execute_query(
        """
        SELECT city_id FROM geocoding_cities
        WHERE address_extraction_setting_id = ? AND city_name = ?
        """,
        (aes_id, city_name),
        fetch_mode="one"
    )
    if dup.get("success") and dup.get("result"):
        return {"success": False, "message": "City already exists for this setting.", "result": []}

    q = """
        INSERT INTO geocoding_cities
            (address_extraction_setting_id, city_name, priority)
        VALUES (?, ?, ?) \
        """
    return db.execute_commit(q, (aes_id, city_name, priority), return_row_id=True)


def delete_geocoding_city(
        db: PostgreSQLDatabase,
        city_id: int | None = None,
        address_extraction_setting_id: int | None = None,
    ...
    """
    Delete a geocoding city.

    Preferred: city_id
    Alternate: (address_extraction_setting_id + city_name)
    """
    if city_id is None and (address_extraction_setting_id is None or not city_name):
        return {
            "success": False,
            "message": "city_id OR (address_extraction_setting_id + city_name) required.",
            "result": []
        }

    if city_id is not None:
        sel_q = "SELECT city_id FROM geocoding_cities WHERE city_id = ?"
        sel_params = (city_id,)
    else:
        sel_q = "SELECT city_id FROM geocoding_cities WHERE address_extraction_setting_id = ? AND city_name = ?"
        sel_params = (address_extraction_setting_id, city_name)
    ...
    cid = sel_res["result"]["city_id"]
    del_res = db.execute_commit(
        "DELETE FROM geocoding_cities WHERE city_id = ?",
        (cid,),
        return_count=True,
        return_row_id=False
    )
    if not del_res.get("success"):
        return {"success": False, "message": del_res.get("message"), "result": []}
    if del_res.get("result", 0) == 0:
        return {"success": False, "message": "No city deleted.", "result": []}

    return {"success": True, "message": "Geocoding city deleted.", "result": []}


def reorder_geocoding_cities(
        db: PostgreSQLDatabase,
        address_extraction_setting_id: int,
        ordered_ids: list[int]
) -> dict:
    """
    Update priority for geocoding cities (top→bottom as 10, 20, 30...).
    """
    if not ordered_ids:
        return {"success": False, "message": "No city IDs provided.", "result": []}
    ...
    for idx, cid in enumerate(ordered_ids):
        priority_val = (idx + 1) * 10
        case_parts.append("WHEN ? THEN ?")
        params.extend([cid, priority_val])

    placeholders = ",".join("?" for _ in ordered_ids)
    case_sql = " ".join(case_parts)
    q = f"""
        UPDATE geocoding_cities
        SET priority = CASE city_id {case_sql} ELSE priority END
        WHERE address_extraction_setting_id = ?
...
def bulk_add_geocoding_cities(
        db: PostgreSQLDatabase,
        address_extraction_setting_id: int,
        cities: list[dict]
) -> dict:
    """
    Bulk insert multiple geocoding cities in a single transaction.
    """
    if not cities:
        return {"success": False, "message": "No cities provided.", "result": []}
    ...
    try:
        ...
        for idx, city in enumerate(cities):
            name = (city.get("city_name") or "").strip()
            priority = int(city.get("priority", (idx + 1) * 10))

            if not name:
                module_logger.warning("Skipping invalid city: %s", city)
                continue

            dup = db.execute_query(
                "SELECT city_id FROM geocoding_cities WHERE address_extraction_setting_id = ? AND city_name = ?",
                (address_extraction_setting_id, name),
                fetch_mode="one"
            )
            if dup.get("success") and dup.get("result"):
                module_logger.warning("City %s already exists, skipping", name)
                continue
            ...
            res = db.execute_commit(
                ...
                VALUES (?, ?, ?)
                """,
                (address_extraction_setting_id, name, priority),
                return_row_id=True
            )
            if not res["success"]:
        ...
        return {
            "success": True,
            "message": f"Added {added} city(ies).",
            "result": added
        }

    except Exception as e:
        module_logger.error("bulk_add_geocoding_cities() rolled back: %s", e)
        db.rollback()
        return {"success": False, "message": str(e), "result": 0}
...
def get_geocoding_roads(
    ...
    """
    Retrieve geocoding roads with optional filters.
    """
    base = """
           SELECT
               gr.road_id,
    ...
    where = " WHERE " + " AND ".join(conditions) if conditions else ""
    query = base + where + " ORDER BY gr.priority ASC, gr.road_name"

    return db.execute_query(query, tuple(params), fetch_mode="all" if road_id is None else "one")


def add_or_update_geocoding_road(db, data: dict) -> dict:
    """
    Insert or update a geocoding road.

    UPDATE path: pass 'road_id'
    INSERT path: omit 'road_id'
    ...
    Optional:
      - road_type, city_name, priority (default 0)
    """
    road_id = data.get("road_id")
    aes_id = data.get("address_extraction_setting_id")
    radio_system_id = data.get("radio_system_id")
    road_name = (data.get("road_name") or "").strip()
    road_type = (data.get("road_type") or "").strip() or None
    ...
    if road_id is not None:
        if not road_name:
            return {"success": False, "message": "road_name required.", "result": []}

        q = """
            UPDATE geocoding_roads
            SET road_name = ?, road_type = ?, city_name = ?, priority = ?
            WHERE road_id = ? \
            """
        return db.execute_commit(q, (road_name, road_type, city_name, priority, road_id), return_count=True)
    ...
    if not road_name:
        return {"success": False, "message": "road_name is required.", "result": []}
    ...
    dup = db.execute_query(
        ...
        """,
        (aes_id, road_name, city_name),
        fetch_mode="one"
    )
    if dup.get("success") and dup.get("result"):
        return {"success": False, "message": "Road already exists for this setting.", "result": []}
    ...
    q = """
        ...
        VALUES (?, ?, ?, ?, ?) \
        """
    return db.execute_commit(q, (aes_id, road_name, road_type, city_name, priority), return_row_id=True)


def delete_geocoding_road(
        db,
        road_id: int | None = None,
        address_extraction_setting_id: int | None = None,
    ...
    Preferred: road_id
    Alternate: (address_extraction_setting_id + road_name + city_name)
    """
    if road_id is None and (address_extraction_setting_id is None or not road_name):
        return {
            "success": False,
            "message": "road_id OR (address_extraction_setting_id + road_name) required.",
            "result": []
        }

    if road_id is not None:
        sel_q = "SELECT road_id FROM geocoding_roads WHERE road_id = ?"
        sel_params = (road_id,)
    else:
        sel_q = "SELECT road_id FROM geocoding_roads WHERE address_extraction_setting_id = ? AND road_name = ? AND (city_name = ? OR (city_name IS NULL AND ? IS NULL))"
        sel_params = (address_extraction_setting_id, road_name, city_name)

    sel_res = db.execute_query(sel_q, sel_params, fetch_mode="one")
    if not sel_res.get("success"):
        return {"success": False, "message": sel_res.get("message"), "result": []}
    if not sel_res.get("result"):
        return {"success": False, "message": "Road not found.", "result": []}

    rid = sel_res["result"]["road_id"]
    del_res = db.execute_commit(
        "DELETE FROM geocoding_roads WHERE road_id = ?",
        (rid,),
        return_count=True,
        return_row_id=False
    )
    if not del_res.get("success"):
        return {"success": False, "message": del_res.get("message"), "result": []}
    if del_res.get("result", 0) == 0:
        return {"success": False, "message": "No road deleted.", "result": []}

    return {"success": True, "message": "Geocoding road deleted.", "result": []}


def reorder_geocoding_roads(
        db,
        address_extraction_setting_id: int,
        ordered_ids: list[int]
) -> dict:
    """
    Update priority for geocoding roads (top→bottom as 10, 20, 30...).
    """
    if not ordered_ids:
        return {"success": False, "message": "No road IDs provided.", "result": []}
    ...
    q = f"""
        UPDATE geocoding_roads
        SET priority = CASE road_id {case_sql} ELSE priority END
        WHERE address_extraction_setting_id = ?
          AND road_id IN ({placeholders})
    """
    params.append(address_extraction_setting_id)
    params.extend(ordered_ids)
...
def bulk_add_geocoding_roads(
        db,
        address_extraction_setting_id: int,
        roads: list[dict]
) -> dict:
    """
    Bulk insert multiple geocoding roads in a single transaction.
    """
    if not roads:
        return {"success": False, "message": "No roads provided.", "result": []}
    ...
    try:
        ...
        for idx, road in enumerate(roads):
            ...
            city_name = (road.get("city_name") or "").strip() or None
            priority = int(road.get("priority", (idx + 1) * 10))
        ...
        module_logger.error("bulk_add_geocoding_roads() rolled back: %s", e)
        db.rollback()
        return {"success": False, "message": str(e), "result": 0}
...
# ===========================================================================
def update_system_storage_settings(db: PostgreSQLDatabase, data: dict) -> dict:
    """
...
def update_system_incident_classification_settings(db: PostgreSQLDatabase, data: dict) -> dict:
    """
...
def delete_system_incident_classification_settings(db: PostgreSQLDatabase, radio_system_id: int) -> dict:
    """
