...
def _dispatch_triggers(
    ...
    # Build shared on-demand map image URL for all notification channels
    ...
    ntfy = NtfySender.from_system_row(system_row, logger=module_logger)
    if ntfy.enabled:
        sent = ntfy.send(ctx, fired_trigger_data=fired_trigger_data)
        module_logger.info("Ntfy dispatched: %d trigger(s)", sent)
    else:
        module_logger.debug("Ntfy disabled for system_id=%s", system_row.get("radio_system_id"))
