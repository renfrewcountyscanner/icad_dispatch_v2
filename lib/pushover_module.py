...
class PushoverSender:
    ...
    def _base_payload(self, *, app_token: str, group_token: str,
        ...
        # Prefer map image URL, fall back to dispatch audio
        map_url = ctx.get("map_image_url")
        url = map_url or ctx.get("audio_url") or ctx.get("mp3_url")
        if isinstance(url, str) and url:
            payload["url"] = url
            payload["url_title"] = "View Map" if map_url else "Dispatch Audio"
