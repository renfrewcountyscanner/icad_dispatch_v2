...
class DiscordSender:
    ...
    def _build_payload_and_files(
        ...
        # ── 1) Map attachment (and embed it as the embed image)
        if self.settings.render_map:
            try:
                map_png = self._maybe_render_map_png(ctx)
                if map_png:
                    map_name = "map.png"
                    files[f"files[{file_index}]"] = (map_name, map_png, "image/png")
                    embed["image"] = {"url": f"attachment://{map_name}"}
                    file_index += 1
            except Exception as e:
                self.log.debug("DiscordSender: map render skipped/failed: %s", e)
    ...
    def _maybe_render_map_png(self, ctx: Dict[str, Any]) -> Optional[bytes]:
        """
        Render a pin map if ctx has unified geocoded coords.
        Expects: ctx["address_lat"] and ctx["address_lng"]
        """
        ...
        # Title/subtitle from ctx (your canonical names)
        title = (ctx.get("incident_type") or ctx.get("incident_category") or "Incident Location").strip()

        subtitle = (
                (ctx.get("address_line") or "").strip()
                or (ctx.get("address_raw") or "").strip()
                or (ctx.get("address_maps_url") or "").strip()
                or None
        )
        ...
        renderer = MapPinRenderer(config=cfg, logger=self.log)
